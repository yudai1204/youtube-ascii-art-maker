...
def make_AA(file_path,str_list="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz +-*/%'"+'"!?#&()~^|@;:.,[]{}<>_0123456789',width=50):
    ...
    gray_img_array=np.asarray(img.convert('L').resize((width,width*img.height//img.width//2)))
    color_img_array=np.asarray(img.resize((width,width*img.height//img.width//2)))
